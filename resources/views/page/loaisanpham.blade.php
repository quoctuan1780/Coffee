									...
									<div class="col-sm-4" style="height: 400px">
										...
										@endif
											<div class="single-item-header">
												<a href="{{route('chi-tiet-san-pham', $sp->id)}}"><img style="height:250px" src="image/product/{{$sp->image}}" alt=""></a>
											</div>
							...
							<div class="beta-products-list">
								...
								@foreach($sp_khac as $sp_k)
									<div class="col-sm-4" style="height: 400px">
										...
										@endif
											<div class="single-item-header">
												<a href="{{route('chi-tiet-san-pham', $sp_k->id)}}"><img style="height:250px" src="image/product/{{$sp_k->image}}" alt=""></a>
											</div>
											<div class="single-item-body">
												<p class="single-item-title">{{$sp_k->name}}</p>
												<p class="single-item-price">
												@if($sp_k->promotion_price == 0)
													<span class="flash-sale">{{number_format($sp_k->unit_price)}} VND</span>
												@else
													<span class="flash-del">{{number_format($sp_k->unit_price)}} VND</span>
													<span class="flash-sale">{{number_format($sp_k->promotion_price)}} VND</span>
												@endif
												</p>
											</div>
											<div class="single-item-caption">
												<a class="add-to-cart pull-left" href="shopping_cart.html"><i class="fa fa-shopping-cart"></i></a>
												<a class="beta-btn primary" href="{{route('chi-tiet-san-pham', $sp_k->id)}}">Chi tiết<i class="fa fa-chevron-right"></i></a>
												<div class="clearfix"></div>
											</div>
										</div>
									</div>
